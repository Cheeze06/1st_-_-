...
void loop()
{
 ...
 if (digitalRead(5) == LOW) {
  ...
  Serial.println(iceame + latte);
  lcd.setCursor(0, 0);
  lcd.print("Total Order");
  lcd.setCursor(0, 1);
  lcd.print(iceame + latte);
  delay(1000);
  lcd.clear();
 }
  ...
  if (content.substring(1)="A0 CB 62 80" || content.substring(1)=="D6 20 C3 C7" )// 승인할 UID들 비교
  {
   // 인증 성공
    ...
    delay(3000);
    lcd.clear();
    lcd.print("Order Prise");
    lcd.setCursor(0, 1);
    lcd.print(iceame * 3000 + latte * 3000);
    delay(3000);
    lcd.clear();
    if (cash >= iceame * 3000 + latte * 3000) {
      lcd.setCursor(0, 0);
      lcd.print("Deal Complate");
      lcd.setCursor(0, 1);
      lcd.print("Thank You");
      delay(3000);
      lcd.clear();
